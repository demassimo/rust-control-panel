...
- `src/discord-bot-utils.js` – Shared Discord-bot helper functions for date parsing, ID sanitisation, count formatting, Discord timestamp rendering, and safe truncation of user-provided ticket text, reused across status, ticketing, and lookup handlers.【F:backend/src/discord-bot-utils.js†L1-L60】【F:backend/src/discord-bot-service.js†L1-L60】
- `src/discord-bot-auth.js` – Extracted Discord/Steam account-linking command handler and related helpers (team auth settings load/save, link token generation, link URL construction) used by the `/auth` slash command in the Discord worker.【F:backend/src/discord-bot-auth.js†L1-L160】【F:backend/src/discord-bot-service.js†L1-L60】
- `src/discord-bot-lookup.js` – Player lookup command module that builds list/detail embeds and queries the DB across all team servers for `/rustlookup player` and `/rustlookup steamid`.【F:backend/src/discord-bot-lookup.js†L1-L160】【F:backend/src/discord-bot-service.js†L4000-L4040】
- `src/audit-log.js` – Thin wrapper around the database audit API that records structured events (source, actor, action, metadata) from both the Discord worker and panel HTTP routes.【F:backend/src/audit-log.js†L1-L24】【F:backend/src/index.js†L6511-L6565】【F:backend/src/discord-bot-service.js†L1-L60】
